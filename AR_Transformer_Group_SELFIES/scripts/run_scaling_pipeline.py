...
def main():
    ...
    tokenizer_path = Path(config['paths']['results_dir']) / 'tokenizer.pkl'
    vocab_size = 100  # Default estimate
    if tokenizer_path.exists():
        with open(tokenizer_path, 'rb') as f:
            tokenizer = pickle.load(f)
            # Group SELFIES tokenizer is a dict with token_to_id
            if hasattr(tokenizer, '__len__'):
                vocab_size = len(tokenizer)
            elif isinstance(tokenizer, dict) and 'token_to_id' in tokenizer:
