"""Hugging Face compatible p-SMILES BPE tokenizer implementation."""
...
class HFSmilesBPETokenizer(PreTrainedTokenizer):
    """HF tokenizer for p-SMILES with learned BPE merges.

    Base tokenization (brackets → ring indices → multi-char atoms → chars) is
    applied first; then the learned BPE merge table is applied on top.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        loaded_vocab = vocab
        loaded_merges: List[Tuple[str, str]] = list(merges) if merges else []
        ...
        if legacy_tokenizer_file:
            ...
            if legacy_path.exists():
                ...
                if not loaded_merges:
                    loaded_merges = [tuple(p) for p in data.get("merges", [])]
        ...
        self.merges: List[Tuple[str, str]] = loaded_merges
    ...
    @staticmethod
    def _merge_pair(tokens: List[str], left: str, right: str) -> List[str]:
        out: List[str] = []
        i = 0
        while i < len(tokens):
    ...
    def _apply_merges(self, tokens: List[str]) -> List[str]:
        for left, right in self.merges:
            tokens = self._merge_pair(tokens, left, right)
        return tokens

    def _tokenize(self, text: str) -> List[str]:
        base = self._tokenize_base(text)
        return self._apply_merges(base) if self.merges else base
    ...
    def build_vocab(self, smiles_list: List[str]) -> Dict[str, int]:
        """Delegate to the underlying PSmilesTokenizer.build_vocab logic inline."""
        from .tokenizer import PSmilesTokenizer
        inner = PSmilesTokenizer(
            max_length=self.max_length,
            bpe_vocab_size=512,
            min_frequency=2,
            max_corpus_size=200000,
        )
        inner.build_vocab(smiles_list)
        self.merges = inner.merges
        self.vocab = inner.vocab
        self.id_to_token = inner.id_to_token
        for tok in self.SPECIAL_TOKENS:
            if tok in self._added_tokens_encoder and tok in self.vocab:
                self._added_tokens_encoder[tok] = self.vocab[tok]
        return self.vocab
    ...
    def verify_roundtrip(self, smiles: str) -> bool:
        return self.detokenize(self._tokenize(smiles)) == smiles
    ...
    @classmethod
    def load(cls, path: Union[str, Path]) -> "HFSmilesBPETokenizer":
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        merges = [tuple(p) for p in data.get("merges", [])]
        return cls(vocab=data["vocab"], max_length=int(data["max_length"]), merges=merges)
...
def load_polymer_tokenizer(
    ...
    for candidate in candidates:
        if candidate.is_dir():
            return HFSmilesBPETokenizer.from_pretrained(str(candidate), local_files_only=True)
        if candidate.is_file():
            return HFSmilesBPETokenizer.load(candidate)
