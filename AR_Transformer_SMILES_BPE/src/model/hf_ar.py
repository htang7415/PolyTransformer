...
def build_polymer_ar_config(
    ...
    tokenizer: Any,
    diffusion_config: Optional[Mapping[str, Any]] = None,
) -> PolymerARConfig:
    ...
    return PolymerARConfig(
        ...
        num_diffusion_steps=int(diffusion_config["num_steps"]) if diffusion_config else 0,
...
def load_polymer_ar_state_dict(
    ...
    try:
        ...
        msg = str(exc)
        if not any(k in msg for k in ("Missing key", "Unexpected key", "size mismatch")):
            raise
